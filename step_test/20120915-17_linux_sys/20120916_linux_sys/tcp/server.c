...
//			printf("nready:%d i:%d  enents.data.fd:%d\n", nready, i, events[i].data.fd);
		...
		//		printf("nready: %d, i=%d\n", nready, i);
				...
				if (p->status == INIT) {				/* 第一次发数据   发送 up  or  down ,如果数据还没结束，来了第二个链接请求，进不到这里 `*/
					n = read(p->cfd, buf, BUFSIZ);
					buf[n] = '\0';						
					printf("..............first translent:%s\n",buf);

					fill_struct(&p->type, &p->status, &p->fd, buf, efd, p->cfd); /* */
			...
			//		print();
					...
					if (events[i].events == EPOLLIN) {  /* 上传文件 */
