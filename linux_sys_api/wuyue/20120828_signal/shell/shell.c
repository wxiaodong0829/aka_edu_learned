...
int do_child(char *args, int id_child, int fd[][2], int n_child)
{
	...
	if (id_child < n_child - 1) {					/* dup2(fd[1], STDOUT_FILENO) 将标准输出重定向到管道写端 */
		if (id_child > 0) {
	...
	if (id_child > 0 && id_child < n_child - 1) {								 /* 重定向此进程的标准输入*/
		close(fd[id_child][0]);// 右边　读

		close(fd[id_child - 1][1]);
		dup2(fd[id_child - 1][0], STDIN_FILENO);
		close(fd[id_child - 1][0]);
	}
	if (id_child == n_child - 1 && id_child != 0) {           /*最右边*/
		close(fd[id_child - 1][1]);
		dup2(fd[id_child - 1][0], STDIN_FILENO);
		close(fd[id_child - 1][0]);
	}
