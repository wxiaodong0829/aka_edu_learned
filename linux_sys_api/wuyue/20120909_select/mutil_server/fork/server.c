...
{
	while (waitpid(-1, NULL, WNOHANG) > 0) /* wait all exit statues child process at this once signal*/
		;
}
...
int main(int argc, char **argv)
{
	...
	while (1) {
		...
		if (pid == 0) { /* child */
			...
			close(cfd);
			exit(0);/* child must should be exit , otherwise this child will still run */
		} else {         /* parent */
			close(cfd);
		}
	}
