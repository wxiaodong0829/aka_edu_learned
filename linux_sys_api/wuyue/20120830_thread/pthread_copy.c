...
//	tid = malloc(sizeof(*tid) * n);
	...
	if ((p->fd_s = open(argv[1], O_RDWR)) == -1)
		error_sys("open src");
	if ((p->fd_d = open(argv[2], O_RDWR | O_CREAT | O_TRUNC, 0644)) == -1)
		error_sys("open dest");
	...
	while (left_size > 0) { //文件结尾处跳出
		...
		p->len = ((int)size)/(n);
		for (i = 0; i < n - 1; i++) {
			pthread_create(&tid[i], NULL, ptr_fn, (void *)p->index); //这是个在变化的值p->index,
			p->index += p->len;
		}

		for (j = 0; j < n - 1; j++) {
			pthread_join(tid[j], NULL);			/*  381 个子线程时出现段错误*/
		}
